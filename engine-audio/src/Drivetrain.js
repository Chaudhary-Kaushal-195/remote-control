import { clamp } from "./util/clamp.js";

export class Drivetrain {

    constructor() {
        this.gear = 0;
        this.clutch = 1.0;
        this.downShift = false;

        this.gears = [3.4, 2.36, 1.85, 1.47, 1.24, 1.07];
        this.final_drive = 3.44;

        this.theta = 0;
        this.omega = 0;
        this.prevTheta = 0;
        this.prevOmega = 0;

        this.theta_wheel = 0;
        this.omega_wheel = 0;

        /* Inertia of geartrain + drive shaft [kg m2] */
        this.inertia = 0.1 + 0.05; /* 0.5 * MR^2 */
        this.damping = 12;
        this.compliance = 0.01;

        this.shiftTime = 50;
        this.isShifting = false;
        this.init();
    }

    init(config) {
        if (config) Object.assign(this, config);

        this.theta = 0;
        this.omega = 0;
        this.prevTheta = 0;
        this.prevOmega = 0;
        this.theta_wheel = 0;
        this.omega_wheel = 0;

        this.gear = 0;
    }

    integrate(dt) {
        this.clutch = clamp(this.clutch, 0, 1);
        this.prevTheta = this.theta;
        this.theta += this.omega * dt;
    }

    update(h) {
        this.prevOmega = this.omega;
        const dTheta = (this.theta - this.prevTheta) / h;
        this.omega = dTheta;
    }

    solvePos(engine, h) {
        const c = engine.theta - this.theta;
        const corr1 = this.getCorrection(c, h, this.compliance);
        this.theta += corr1 * Math.sign(c);
    }

    solveVel(engine, h) {
        let damping = this.damping;

        if (this.gear > 3)
            damping = this.damping * 0.75;

        this.omega += (engine.omega - this.omega) * damping * h;
    }

    getCorrection(corr, h, compliance = 0) {
        const w = corr * corr * 1 / this.inertia;
        const dlambda = -corr / (w + compliance / h / h);
        return corr * -dlambda;
    }

    getFinalDriveRatio() {
        return this.final_drive;
    }

    getGearRatio(gear = this.gear) {
        // Fix for negative gear mapping for reverse
        if (gear === -1) return -3.4; // Reverse 

        gear = clamp(gear, 0, this.gears.length);
        const ratio = gear > 0 ? this.gears[gear - 1] : 0;
        return ratio;
    }

    getTotalGearRatio() {
        return this.getGearRatio() * this.getFinalDriveRatio();
    }

    changeGear(gear) {
        if (this.isShifting) return;

        const prevRatio = this.getGearRatio(this.gear);
        const nextRatio = this.getGearRatio(gear);

        // Fix: Use Math.abs because nextRatio / prevRatio can be negative when shifting into reverse, causing broken math
        const ratioRatio = prevRatio !== 0 ? Math.abs(nextRatio / prevRatio) : 0;

        if (this.gear === gear && ratioRatio === 1)
            return;

        /* Neutral */
        this.gear = 0;
        this.isShifting = true;

        if (ratioRatio > 1)
            this.downShift = true;

        /* Engage next gear */
        setTimeout(() => {
            this.omega = this.omega * ratioRatio;

            // Fix: we must allow gear to be -1 for reverse, but Drivetrain.ts original code clamped to 0..gears.length!
            if (gear === -1) {
                this.gear = -1;
            } else {
                this.gear = clamp(gear, 0, this.gears.length);
            }

            this.downShift = false;
            this.isShifting = false;
        }, this.shiftTime)
    }

    nextGear() {
        this.changeGear(this.gear + 1);
    }

    prevGear() {
        this.changeGear(this.gear - 1);
    }
}
